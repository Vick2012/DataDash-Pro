...
export default function UsoMaquinasChart({ data }: Props) {
  ...
  const chartData = useMemo(() => {
    const base = [...safeData].sort((a, b) => (order === 'desc' ? b.horas - a.horas : a.horas - b.horas));
    ...
    <div>
      ...
      <div style={{ height: chartHeight }} className="chart-scroll-x">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart layout="vertical" data={chartData} margin={{ top: 6, right: 16, left: 4, bottom: 6 }}>
            ...
            <Tooltip
              ...
              labelFormatter={(value) => `Máquina: ${value}`}
            />
            <Bar dataKey="horas" fill="#8b5cf6" radius={[0, 4, 4, 0]} barSize={18} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
